<?php
session_start();
?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<meta name="veiwport" content="width=device-width, initial-scale=1.0">
<meta name="description" content="learning menagment system for nsbm">
	<title>management</title>
	<link rel="stylesheet" type="text/css" href="CSS\demo.css">
  <link rel="stylesheet" type="text/css" href="CSS\Computing.css">
</head>
<body>
<header>
<!-- navgration bar -->
		<div class="nav">
			<ul>
  				<li><a  class="active" href="Alogin.php">LMS-HOME</a></li>
  				<li><a href="AboutUS.php">About US</a></li>
  				<li><a href="News.php">NEWS</a></li>
  				<li><a href="Clubs.php">Clubs & socities</a></li>
  				<li><a href="ContactUS.php">CONTACT US</a></li>
                 <li>
                    <?php
                    if (isset($_SESSION['uname'])){echo'
                    <a href="logOut.php">LOGOUT</a>';}
                                                   ?>
                </li>
                <li><?php
                    if (isset($_SESSION['uname'])){
                        echo '<a class="my"> You Logged as: '.$_SESSION['uname'].'<a>';
                    }  
                    else {
                        echo '<a>You are not logged in !<a>';
                    }

                    ?>
                </li>  
			</ul>

		</div>
</header>
    <div class="img">
      <div>
        <h1 id="welcome2">School of Business</h1>
      </div>
        <div class="con">
         <div class="computing">
             <ul>Year 1
             <li><a href="">Economics</a></li>
             <li><a href="">Financial Accounting</a></li>
             <li><a href="">Management Process</a></li>
             <li><a href="">Business Communication</a></li>
        </ul>
      </div>
        
             <div class="computing">
             <ul>Year 2
             <li><a href="">Logistics Management</a></li>
             <li><a href="">Marketing Management</a></li>
             <li><a href="">Professional Development</a></li>
             <li><a href="">Organization Behaviour</a></li>

        </ul>
      </div>        
            <div class="computing">
             <ul>Year 3
             <li><a href="">Procurement Management</a></li>
             <li><a href="">Warehouse Management</a></li>
             <li><a href="">Time Management</a></li>
             <li><a href="">Scope Management</a></li>
             
        </ul>
      </div>
            <div class="computing">
             <ul>Year 4
             <li><a href="">Employee Resourcing</a></li>
             <li><a href="">Cost Management</a></li>
             <li><a href="">Project HR Management</a></li>
            <li><a href="">Project Monitoring</a></li>
             
        </ul>
      </div>
        </div>
    </div>
    </body>
</html>
<?php
session_start();
?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<meta name="veiwport" content="width=device-width, initial-scale=1.0">
<meta name="description" content="learning menagment system for nsbm">
	<title>Clubs</title>
	<link rel="stylesheet" type="text/css" href="CSS\demo.css">
  <link rel="stylesheet" type="text/css" href="CSS\ModuleStyle.css">
    <link rel="stylesheet" type="text/css" href="CSS\Clubs.css">
</head>
<body>
<header>
<!-- navgration bar -->
		<div class="nav">
			<ul>
  				<li><a  class="active" href="Alogin.php">HOME</a></li>
  				<li><a href="AboutUS.php">About US</a></li>
  				<li><a href="News.php">NEWS</a></li>
  				<li><a href="Clubs.php">Clubs & socities</a></li>
  				<li><a href="ContactUS.php">CONTACT US</a></li> 

                <li>
                    <?php
                    if (isset($_SESSION['uname'])){echo'
                    <a href="logOut.php">LOGOUT</a>';}
                                                   ?>
                </li>
                <li><?php
                    if (isset($_SESSION['uname'])){
                        echo '<a> You Logged as: '.$_SESSION['uname'].'<a>';
                    }  
                    else {
                        echo '<a>You are not logged in !<a>';
                    }

                    ?>
                </li>  
			</ul>

		</div>
</header>
    <div class="img">
      <div>
        <h1 id="welcome2">Clubs & Societies</h1>
      </div>
        <div class="con">
            <div>
            <p class="clubs">NSBM Green University Town is a thriving network of students, staff and alumni. With the aim of developing the soft skills of the students, a range of activities have been initiated through various societies and clubs</p>
            </div>
            <div class="clubs1">
            <ul>
                <li>Media Club</li>
                <li>Eastern Music Club</li>
                <li>Western Music Club</li>
                <li>Drama Club</li>
                <li>Dancing Club</li>
                <li>Explorers’ Club</li>
                <li>Green Task Force</li>
                <li>Nature & Wild Life Club</li>
                <li>Photography Club</li>
                <li>Literature Club</li>
                <li>Graphics and Painting Club</li>
                <li>Debate Club</li>
            </ul> 
            </div>
            <div class="clubs1">
                <ul>
                    <li>Buddhist Society</li>
                    <li>Hindu Society</li>
                    <li>Islamic Society</li>
                    <li>Catholic Society</li>
                </ul>
            </div>
            <div class="clubs1">
                <ul>
                    <li>Rotaract Club</li>
                    <li>LEO Club</li>
                    <li>AIESEC</li>
                    <li>Gavel Club</li>
                    <li>Toastmaster Club</li>
                </ul>
            </div>
        </div>
    </div>
    </body>
</html>